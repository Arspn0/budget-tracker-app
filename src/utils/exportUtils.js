    ...
    <tr>
      <td>
        <span style="display:inline-block;width:10px;height:10px;border-radius:50%;
          background:${cat.color || '#9FA5B4'};margin-right:6px;"></span>
        ${cat.name}
      </td>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Helvetica Neue', Arial, sans-serif; color: #1a1a2e; background: #f8f9fa; padding: 32px; }
  .header { background: linear-gradient(135deg, #3ED6C4, #17A696); color: white; border-radius: 16px; padding: 28px 32px; margin-bottom: 28px; }
  ...
  .summary-card { background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,.06); }
  .summary-card .label { font-size: 12px; color: #9FA5B4; font-weight: 600; text-transform: uppercase; letter-spacing: .5px; }
  .summary-card .value { font-size: 22px; font-weight: 800; margin-top: 6px; }
  .success { color: #4CAF50; }
  .danger  { color: #FF5252; }
  .neutral { color: #3ED6C4; }
  section { background: white; border-radius: 12px; padding: 24px; margin-bottom: 24px; box-shadow: 0 2px 8px rgba(0,0,0,.06); }
  ...
  th { text-align: left; color: #9FA5B4; font-weight: 600; font-size: 11px; text-transform: uppercase; letter-spacing: .5px; padding: 8px 10px; border-bottom: 1px solid #f0f0f0; }
  td { padding: 12px 10px; border-bottom: 1px solid #f8f8f8; vertical-align: middle; }
  tr:last-child td { border-bottom: none; }
  .right { text-align: right; }
  .footer { text-align: center; color: #9FA5B4; font-size: 12px; margin-top: 32px; }
</style>
</head>
<body>
...
<section>
  <h2>💳 Saldo Dompet</h2>
  <table>
    <thead><tr><th>Nama</th><th>Tipe</th><th class="right">Saldo</th></tr></thead>
...
<section>
  ...
  <table>
    <thead><tr><th>Kategori</th><th class="right">Jumlah Transaksi</th><th class="right">Total</th></tr></thead>
    <tbody>${catRows}</tbody>
  </table>
</section>` : ''}
...
<section>
  ...
  ${transactions.length === 0
    ...
    : `<table>
    <thead><tr><th>Tanggal</th><th>Kategori</th><th>Dompet</th><th>Catatan</th><th class="right">Jumlah</th></tr></thead>
    <tbody>${txRows}</tbody>
  </table>`}
</section>

<div class="footer">Budget Tracker App &nbsp;·&nbsp; Laporan dibuat otomatis</div>
</body>
</html>`;
};
...
export const exportPDF = async ({ transactions, summary, categoryBreakdown, wallets, month, year }) => {
  try {
    ...
    // Move to a readable location with a nice filename
